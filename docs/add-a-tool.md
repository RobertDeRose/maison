...
untracked work, and rejects a dirty target declaration or lockfile. A successful mutation commits only its declaration
and generated lock paths with a subject such as `added(tool): \`github:owner/tool@version\``. If Git commit creation
fails, the validated files remain in place for manual recovery. The refresh is not full `maison sync`.
